...
class JobOffers {
  ...
  constructor(employer: string, position: string) {
    this.employer = employer
    this.position = position
  }

  jobApplication(candidates: string[]): string {
    ...
    for (let candidate of candidates) {
      ...
      const candidateIndex = this.jobCandidates.findIndex(
        (c) => c.name === name
      )
      if (candidateIndex === -1) {
        ...
        if (
          Number(this.jobCandidates[candidateIndex].yearsExperience) <
  ...
  jobOffer(chosenPerson: string): string {
    const [name, minimalExperience] = chosenPerson.split("-")
    const personIndex = this.jobCandidates.findIndex(
  ...
  salaryBonus(name: string): string {
    const personIndex = this.jobCandidates.findIndex(
      (cand) => cand.name === name
    )
    if (personIndex === -1) {

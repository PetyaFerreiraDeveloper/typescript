...
class ArtGallery {
    ...
    addArticle(articleModel, articleName, quantity) {
        ...
        articleModelLowerCase;
        const articleIndex = this.listOfArticles.findIndex((article) => articleName === article.articleName);
        if (articleIndex === -1 ||
    ...
    inviteGuest(guestName, personality) {
        ...
        const guestPoints = personalityTable[personality] ?? 50;
        this.guests.push({ guestName, points: guestPoints, purchaseArticle: 0 });
        return `You have successfully invited ${guestName}!`;
    }
    buyArticle(articleModel, articleName, guestName) {
        const articleIndex = this.listOfArticles.findIndex((article) => article.articleName === articleName);
        if (articleIndex === -1 ||
            this.listOfArticles[articleIndex].articleModel !== articleModel)
        ...
        const currentGuestIndex = this.guests.findIndex((guest) => guest.guestName === guestName);
